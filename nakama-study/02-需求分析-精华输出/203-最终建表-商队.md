...
COMMENT ON TABLE caravan_roles IS '商队成员职业定义：护卫/账房/郎中/向导/脚夫/货匠';
```
商队成员 npc 模板 的配置，记录 npc 初始信息
```sql
CREATE TABLE IF NOT EXISTS crew_npc_template (
    crew_npc_template_id SERIAL PRIMARY KEY,
    code STRING NOT NULL UNIQUE,
    `name` STRING NOT NULL,  ---- NPC 名称（苏明远、秦铁山等）
...
COMMENT ON COLUMN crew_template.crew_npc_template_id IS 'npc主键';
COMMENT ON COLUMN crew_template.code IS '模板编码，例如 ACCOUNTANT_SU_MINGYUAN';
COMMENT ON COLUMN crew_template.name IS 'NPC 名称（苏明远、秦铁山等）';
COMMENT ON COLUMN crew_template.role_code IS '职业编码：guard/accountant/doctor/guide/porter/custodian';
COMMENT ON COLUMN crew_template.strength IS '基础力量';
COMMENT ON COLUMN crew_template.agility IS '基础敏捷';
COMMENT ON COLUMN crew_template.constitution IS '基础体质';
COMMENT ON COLUMN crew_template.wisdom IS '基础智慧';
COMMENT ON COLUMN crew_template.wage IS '基础工资（两）';
...
COMMENT ON COLUMN character_caravan_members.is_hired IS '是否处于已加入商队的状态（未解雇）';
```
